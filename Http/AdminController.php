...
class AdminController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
			...
			$query = $this->applyScope($model, $dataType, 'index');
    ...
    public function show(Request $request, $id)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
			...
			$this->applyScope($model, $dataType, 'show');
    ...
    public function edit(Request $request, $id)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
			...
			$this->applyScope($model, $dataType, 'edit');
    ...
    public function update(Request $request, $id)
    {
		...
		$this->applyScope($model, $dataType, 'update');
		...
		event(new BreadDataBeforeUpdated($dataType, $data));
        ...
        return $this->getSuccessRedirect('update', $dataType, $data, [
			'success' => __('voyager::generic.successfully_updated')." {$dataType->getTranslatedAttribute('display_name_singular')}"
		]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!$request->has('_tagging')) {
            ...
            return $this->getSuccessRedirect('store', $dataType, $data, [
                'success' => __('voyager::generic.successfully_added_new')." {$dataType->getTranslatedAttribute('display_name_singular')}",
			]);
        } else {
    ...
    public function destroy(Request $request, $id)
    {
        ...
        } catch (\F9Web\LaravelDeletable\Exceptions\NoneDeletableModel $ex) {
            $message = [
                'error'    => $ex->getMessage(),
            ];
            return $this->getSuccessRedirect('destroy', $dataType, $data, $message);
        }

        $message = $res
            ? [
                'success'    => __('voyager::generic.successfully_deleted')." {$displayName}",
            ]
            : [
                'error'    => __('voyager::generic.error_deleting')." {$displayName}",
            ];
    ...
    public function restore(Request $request, $id)
    {
		...
		$this->applyScope($model, $dataType, 'restore');
        ...
        $message = $res
            ? [
                'success'    => __('voyager::generic.successfully_restored')." {$displayName}",
            ]
            : [
                'error'    => __('voyager::generic.error_restoring')." {$displayName}",
            ];
        ...
        return $this->getSuccessRedirect('restore', $dataType, $data, $message);
    }
	...
	protected function getSuccessRedirect($action, $dataType, $data = null, array $message = null) {
		...
		} else if ($action == 'destroy' || $action == 'restore') {
			$redirect = redirect()->route("voyager.{$dataType->slug}.index");
		}
		...
		if (isset($message)) {
			$redirect->with([
				...
				'alert-type' => key($message),
			]);
		}
		return $redirect;
	}
	
	protected function applyScope($model, $dataType, $action) {
		...
		// Search Model
		if ($model->getModel() instanceof \Ant\Core\Interfaces\Searchable) {
			$model = $model->search(request());
		}
